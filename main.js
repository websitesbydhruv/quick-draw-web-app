...
function preload() {
    classiier = ml5.imageclassifier('DoodleNet');
}
    ...
    function check_sketch() { timer_counter++; 
        document.getElementById('time').innerHTML = 'Timer: ' + timer_counter; console.log(timer_counter)
         if (timer_counter > 400) { timer_counter = 0; timer_check = "completed" }
          if (timer_check == "completed" || answer_holder == "set") { timer_check = "";
...
function gotResult(error, results) {
    if (error) {
        ...
        document.getElementById("drawn").innerHTML = "Your sketch:" + drawn_sketch;
